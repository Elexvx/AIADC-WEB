...
SELECT 1001, 'system:update:install', '瀹夎骞冲彴鏇存柊', 'system', 'CORE', NULL, 0, CURRENT_TIMESTAMP, 0, CURRENT_TIMESTAMP, 0
WHERE NOT EXISTS (SELECT 1 FROM sys_permission WHERE tenant_id = 1001 AND permission_key = 'system:update:install');

INSERT INTO sys_permission (tenant_id, permission_key, permission_name, permission_group, source_type, plugin_code, created_by, created_at, updated_by, updated_at, deleted)
SELECT 1001, 'system:update:rollback', '鍥炴粴骞冲彴鏇存柊', 'system', 'CORE', NULL, 0, CURRENT_TIMESTAMP, 0, CURRENT_TIMESTAMP, 0
