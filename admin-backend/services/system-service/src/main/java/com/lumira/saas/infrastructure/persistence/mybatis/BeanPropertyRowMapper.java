...
public class BeanPropertyRowMapper<T> implements RowMapper<T> {
    ...
    public T mapRow(SqlRow row, int rowNum) {
        ...
        for (Map.Entry<String, Object> entry : row.asMap().entrySet()) {
            String propertyName = resolvePropertyName(wrapper, normalizePropertyName(entry.getKey()));
    ...
    private String resolvePropertyName(BeanWrapper wrapper, String propertyName) {
        if (wrapper.isWritableProperty(propertyName)) {
            return propertyName;
        }
        for (PropertyDescriptor descriptor : wrapper.getPropertyDescriptors()) {
            if (descriptor.getWriteMethod() != null && descriptor.getName().equalsIgnoreCase(propertyName)) {
